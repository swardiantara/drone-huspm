...
class DroneLogAnalyzer:
    ...
    def analyze(self):
        ...
        cluster_member_path = os.path.join(output_dir, f'{self.config['filename']}_cluster_mapping')
        self.abstractor.save_cluster_member(f'{cluster_member_path}_sentence.json', f'{cluster_member_path}_message.json')
        output_dir = os.path.join(self.config['workdir'], 'event')
        os.makedirs(output_dir, exist_ok=True)
        event_path = os.path.join(output_dir, f'{self.config['filename']}_event')
        self.abstractor.save_representative_log(f'{event_path}_sentence.json', f'{event_path}_message.json')
        output_dir = os.path.join(self.config['workdir'], 'problem')
        os.makedirs(output_dir, exist_ok=True)
        problem_path = os.path.join(output_dir, f'{self.config['filename']}_problem')
        self.abstractor.save_problem(f'{problem_path}_sentence.json', f'{problem_path}_message.json')
        ...
        output_dir = os.path.join(self.config['workdir'], 'message_report')
        os.makedirs(output_dir, exist_ok=True)
        self.report_generator.create_timeline_chart(records, self.abstractor.message_problem['binary'], output_dir, 'message')
        df_message.to_excel(os.path.join(output_dir, f'{self.config['filename']}_message.xlsx'))
        output_dir = os.path.join(self.config['workdir'], 'sentence_report')
        os.makedirs(output_dir, exist_ok=True)
        self.report_generator.create_timeline_chart(records, self.abstractor.sentence_problem['binary'], output_dir, 'sentence')
